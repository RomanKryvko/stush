...
TEST(ParserTest, IgnoresBlankLines) {
    args_container exp {};
    auto blank = tokenizer::tokenize("         ", " ");
    EXPECT_EQ(exp, blank);
    auto empty = tokenizer::tokenize("", " ");
    EXPECT_EQ(exp, empty);
}

TEST(ParserTest, LeadingAndTrailingDelimiters) {
    args_container exp {"test", "arg1", "arg2"};
    auto res = tokenizer::tokenize("   test arg1 arg2  ", " ");
    EXPECT_EQ(exp, res);
}

TEST(ParserTest, SingleWord) {
    args_container exp {"test"};
    auto res = tokenizer::tokenize("test", " ");
    EXPECT_EQ(exp, res);
}
...
TEST(ParserTest, ConsecutiveDelimitersAreIgnored) {
    ...
    auto res = tokenizer::tokenize("a   b    c", " ");
    EXPECT_EQ(exp, res);
}

TEST(ParserTest, QuotesGroupWords) {
    args_container exp {"a", "\"b   c   d\"", "e"};
    auto res = tokenizer::tokenize("a  \"b   c   d\"    e   ", " ");
    EXPECT_EQ(exp, res);
}
...
TEST(ParserTest, EmptyQuotesBecomeEmptyArg) {
    ...
    auto res = tokenizer::tokenize("a \"\" b", " ");
    EXPECT_EQ(exp, res);
}
...
TEST(ParserTest, SplitsOutDoubleCommandSeparators) {
    args_container exp {"echo", ";", "exit", "&&", "echo", "|", "wc", "-l",
        "||", "exit"};
    auto res = tokenizer::tokenize("echo;exit&&echo| wc -l || exit", " ");
    EXPECT_EQ(exp, res);
}
...
TEST(ParserTest, HandlesCommandSeparatorsAtTheEnd) {
    ...
    exp = {"echo", "&&", "exit", ";", ";", ";", "&&"};
    res = tokenizer::tokenize("  echo&&  exit;;;     &&       ", " ");
    EXPECT_EQ(exp, res);
